...
% sarImage = fftshift(fft2(sarData));
...
%%
% maxEle=max(max(abs(sarImage)));
% minEle=min(min(abs(sarImage)));
% filterEle = (maxEle+minEle)*0.5;
% sarImage(abs(sarImage)>=filterEle) = maxEle;
% sarImage(abs(sarImage)<filterEle) = minEle;



%%
%test = fliplr(sarImage);
%%
% Plot SAR Image
%target = abs(fliplr(sarImage));
% test = fft2(sarData(512,309:715));
% target = abs(fliplr(fftshift(ifft2(test))));
% figure; 
% plot(target);

target = abs(fliplr(sarImage));
figure; 
mesh(xRangeT,yRangeT,target,'FaceColor','interp','LineStyle','none')
view(2)
colorbar
colormap('jet');
...
% %%
% figure;
% subplot(211):plot(sarImage);
% subplot(212):polarplot(angle(sarImage),abs(sarImage));
