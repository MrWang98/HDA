...
isReal = 0;
% set to 1 if real only data, 0 if complex dataare populated with 0 
%% read file andconvert to signed number
% read .bin file
...
%     adcData2 = tall(adcData);
    x = adcData([5,6,7,8],:);
    adcData([5,6,7,8],:)=[];
    adcData = adcData([1,2,3,4],:) + sqrt(-1)*x;
end
